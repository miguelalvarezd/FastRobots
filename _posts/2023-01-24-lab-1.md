...
- `send_command(cmd_type, data)`, used to send a command to the board and some type of data. It needs the type of command, and it may need some data;
...
- `start_notify(uuid, notification_handler)`, used to activate notifications, as will be explained later on. It also needs a UUID, and a function that will be executed;
...
This task involves creating a notification handler. A notification handler is a function that gets executed every time the Artemis sends a message, without the need to explicitly run the command `receive_string(uuid)`. To set up the notification handler, we first need to define a function that will be executed (in this case, `notification_handler_3`), and then call the function `start_notify` to activate notifications. The `notification_handler_3` function must receive the UUID and the byte array as arguments; while the `start_notify` function needs the UUID and the notification handler function. The notification handler will convert the byte array sent by the Artemis into a string, and then extract the time as a float. I will store both the string and the time as global variables in case I later want to display them or work with them. The notification handler will look as follows:
```
...
```
Note that I have added some code before calling the `start_notify` function. Its purpose is to stop notifications previously started, before running this one. This way, I can write different notification handlers for the different tasks of this lab, without the need to restart the board or the Python kernell, and so that they don't overlap one another. Now, after starting the notification, whenever I send the command `GET_TIME_MILLIS`, the computer will receive it and print it without the need to run `receive_string(uuid)`.
...
For this task, we have to create a loop that collects the time and sends it to the computer for a few seconds. I will create a new command, `GET_TIME_MILLIS_LOOP`, that will collect the time and send it to the computer repeatedly, during 5 seconds. The code will have the following structure:
```
...
```
Additionally, I will modify the notification handler so that it stores every time stamp in a list, as shown in this piece of code:
```
time_stamps.append(float(string[2:]))
```
Therefore, before sending the command, I will empty the list. After the command is sent, all the time stamps will be stored in the list. When the job is done, I will print the number of time stamps inside the list with `len(time_stamps)`. This way I will know how many messages have been sent in those 5 seconds.

<img src="{{ site.baseurl }}/assets/images/lab1/loop.png" alt="connection.yaml file.">

In my case, the board sent 341 messages in 5 seconds. Each message was a string of 8 characters; which means each string is 64 bits. Therefore, the data transfer rate is (21824 bits / 5 seconds)= 4365 bits/s = 4.26kbps. (or 0.53 kBps)

#### 5. SEND_TIME_DATA command
This task is similiar to the previous one. However, instead of collecting a data point and sending it right away, we will collect all the data points, store them in an array, and then, loop through the array to send all those values. For this example I will collect 10 data points (`#define N 10`). The code for this new command, `SEND_TIME_DATA`, will have the following structure:
```
for(int i=0; i<N; i++){
...
```
The notification handler for this case is similar to the previous one: all the data received is stored into a list. However, this time I will print the whole list when the task is ended. Note that all the data points are 500ms apart. That is becaused I have added a delay whe storing the time stamps in the array, because otherwise the Artemis will be quite fast and collect the same time into various data points.

<img src="{{ site.baseurl }}/assets/images/lab1/send_data.png" alt="connection.yaml file.">

#### 6. GET_TEMP_READINGS command
The last command for the lab, `GET_TEMP_READINGS`, is an increment over the previous one. This time, we will record the temperature at the same time that we record the time; and store both in different arrays. This time, there will be a 1 second delay between every recording. To record the temperature we will use the command `getTempDegC()`, that will return a float value. Then, we will loop through both arrays and send both data points in a single string. The string sent in this case will have the following structure: `Time:[time] Temp:[temperature]`.
```
for(int i=0; i<N; i++){
...
```
The notification handler in this task varies a bit. As we are sending two data points in a single string, in order to parse the data we need to use the `split()` function, and then store both values in their respective arrays.
```
...
```
Later on, after sending the command, and when the task is ended, we will print the time stamp with its respective temperature.

<img src="{{ site.baseurl }}/assets/images/lab1/temp.png" alt="connection.yaml file.">

#### 7. Differences between Methods
The first method to record data, while slower, gives us the ability to receive data in real time. The second method is faster, as it doesn't have to send the data before recording again, but we lose the real time capabilities. In order to determine how fast is the second method, I will modify slightly the command in task 6, so that there's no delay between recordings, but it will record 100 times, instead of 10. In this case, the first time stamp is 18.452s, and the 100th is 18.480s. Therefore, data has been recorded every 0.28ms. Now, let's consider the previous task, where every string had 22 characters (the time stamp and temperature recording have 6 characters each). This means every data point is 22 bytes. If the Artemis has 384kB of RAM, we could store up to 17873 data points.
