...
The last command for the lab, `GET_TEMP_READINGS`, is an increment over the previous one. This time, we will record the temperature at the same time that we record the time; and store both in different arrays. To record the temperature we will use the command `getTempDegC()`, that will return a float value. Then, we will loop through both arrays and send both data points in a single string.
```
for(int i=0; i<N; i++){
