...
I will run a test with a very fast (and not optimal) Proportional controller with Kp=4 to try to reach the maximum angular speed that the motors can provide, and record that data, for different values of myFSS.g. I will change this parameter with the first argument received from Bluetooth (a number from 1 to 4, corresponding to each limit). We can see that the speed is capped in both dps250 and dps500; but it doesn't get past 1000dps. Therefore, I will set the maximum angular speed to 1000dps.
