...
<ul> <li> ::marker "The speed we set to the motor. In this case, following the error sign convention, the right motor will receive the positive value of the PID, while the left one will receive the negative value. Again, if the PID is small but not 0, round up the value to 1." </li></ul>
